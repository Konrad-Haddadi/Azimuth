...
namespace Azimuth
{
	public class Config
	{
		...
		public static VALUE? Get<VALUE>(string _category, string _key)
		{
			...
			Console.WriteLine($"[Error] Attempted to get config value for type '{valueType}' with key '{_key}' int '{_category}'.");
		...
		private void Load()
		{
			...
			// If the directory doesnt exit, create it and return
			...
			using(StreamReader reader = new StreamReader(FilePath))
			{
				...
				// line will only be null if we reached the end of the fline
				// empty lines arent considered null
